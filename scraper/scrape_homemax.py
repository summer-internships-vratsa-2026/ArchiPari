...
PRODUCTS = {
    ...
    "gipsokarton-sonicboard-12-5-1200x2000": {
        ...
        # ВАЖНО: HomeMax НЕ продава звукоизолиращия Sonicboard, а само
        # влагоустойчив тип H2 - различен продукт по предназначение.
        # Продажба само в магазините, не онлайн.
    },
...
def parse_price(html: str, page_url: str, unit_size: float | None = None):
    ...
    Логика:
    1) Ако продуктът има зададен "unit_size" (т.е. реално искаме цена за
       базова единица, не за цялата опаковка), първо пробваме да намерим
       директно показана от HomeMax цена за базовата единица (напр. "Цена
       за кв.м.") за същия продукт.
    2) Ако не успеем (или "unit_size" изобщо не е зададен — значи
       опаковката/бройката Е базовата единица, напр. чувал цимент или
       гипсокартон продаван "на брой"), вземаме цената на цялата опаковка
       в евро и, ако има "unit_size", я разделяме на него.

    ПОПРАВЕНО: по-рано _find_own_base_unit_price() се извикваше ВИНАГИ,
    дори когато продуктът изобщо няма "unit_size" зададен (напр.
    гипсокартон, продаван "на брой") — HomeMax показва "Цена за кв.м."
    в картичка за същия продукт почти навсякъде по страницата, затова
    скрапера грешно вземаше нея (3.73 €/м²) вместо реалната цена за
    бройка (8.94 € за 2.4 м² на брой). Сега базовата единица се търси
    само когато продуктът наистина е конфигуриран за нея.
    """
