...
PRODUCTS = {
    ...
    "mazilka-silikonova-b2-vlachena-25kg": {  # <- същото id във всички скрапери за този продукт!
        # ЗАБЕЛЕЖКА: цената не се показва статично в HTML-а (JS-рендирана,
        # виж fetch_rendered_price по-долу) — цената по-долу в
        # data/products.json (48.62 €, от 95.09 лв.) е взета чрез търсене
        # в момента на добавяне, а не автоматично от този скрапер. Провери
        # я ръчно при следващо пускане.
        "url": f"{BASE_URL}/product/20950-mazilka-silikonova-deko-v2-25kg",
